...
from app.models.user_business_entity_model import (
    ...
    UpdateUserBusinessEntityModel, UserBusinessEntityModel
)
...
class IUserBusinessEntityPostgresRepository(Protocol):

    async def get_user_business_entity_by_id(self, user_id: UUID, user_business_entity_id: UUID) -> UserBusinessEntity| None:
        ...

    async def get_multiple_user_business_entities_by_ids(self, user_id: UUID, user_business_entities_ids: tuple[UUID, ...]) -> tuple[UserBusinessEntity, ...]:
        ...

def new_user_business_entity_postgres_repository() -> IUserBusinessEntityPostgresRepository:
    try:
        return UserBusinessEntityPostgresRepository()
    except Exception as e:
        raise DatabaseError(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
            message=f"Unexpected error occurred while creating user business entity postgres repository.",
            argument=None,
            child_error=e
        )

class UserBusinessEntityPostgresRepository(BasePostgresRepository, IUserBusinessEntityPostgresRepository):
    ...
    async def get_user_business_entity_by_id(self, user_id: UUID, user_business_entity_id: UUID) -> UserBusinessEntity | None:
        try:
            ...
            user_business_entity: UserBusinessEntity | None = await self.session.scalar(stmt)
            return user_business_entity
        except Exception as e:
            raise DatabaseError(
                status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
                message=f"Unexpected error occurred while getting user business entity by id.",
                argument=None,
                child_error=e
            )

    async def get_multiple_user_business_entities_by_ids(self, user_id: UUID, user_business_entity_id: tuple[UUID, ...]) -> tuple[UserBusinessEntity, ...]:
        try:
            stmt = (
                select(UserBusinessEntity).
                where(
                    UserBusinessEntity.id.in_(user_business_entity_id),
                    UserBusinessEntity.user_id == user_id
                )
            )
            user_business_entity: UserBusinessEntity | None = await self.session.scalar(stmt)
